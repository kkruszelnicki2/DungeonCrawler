...
int main()
{
    ...
    while (window.isOpen())
    {
        ...
        if(time)
        {
            ...
            {
                //atak boomera
                if(monsters[i]->detonate)
                {
                    if(monsters[i]->detonate==2)
                    {
                        if(monsters[i]->acceleration.getElapsedTime().asMilliseconds()>100)
                        {
                        ...
                        else
                        {
                            ...
                            {
                                if(monsters[i]->hitbox.getGlobalBounds().intersects(monsters[j]->hitbox.getGlobalBounds()) && i!=j)
                                {
                                    monsters[j]->set_health(50);
                                    monsters[j]->damaged();
                                }
                            }
                        }
