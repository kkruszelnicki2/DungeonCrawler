...
void Boomer::death(std::vector<pickups*> &pickups, int coin_chance, int key_chance)
{
    ...
    {
        pickups.push_back(new coin(hitbox_pos_x+size_x/2,hitbox_pos_y+size_y/1.5 - 10));
    }
    else if(chance > 50 && chance < 50+key_chance)
    {
        pickups.push_back(new key(hitbox_pos_x+size_x/2,pos_y+size_y/1.5 - 6));
    }
}
...
int Boomer::base_attack = 10;
int Boomer::base_health = 100;
