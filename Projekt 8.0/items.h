...
{
    name = "mask_health";
    pickup_texture.loadFromFile(mask_dir);
...
void mask_health::pick(hero *Hero)
{
    ...
    Hero->set_r_maskAmount(1);  // maska na zycie
}
...
void mask_attack_damage::pick(hero *Hero)
{
    ...
    Hero->set_w_maskAmount(1);  // maska na atak
}
...
void mask_speed::pick(hero *Hero)
{
    if(Hero->get_coinAmount()>=worth)
    {
        ...
        Hero->set_g_maskAmount(1);  // maska na predkosc
    }
}
