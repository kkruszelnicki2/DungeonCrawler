    ...
    for(int i = 0; i < 4; i++)
    {
        if(i == 0)
        {
            if(is_room[x][y-1].can_enter())
            {
                if(!fight_mode)
                {
                    drzwi[i].setTextureRect(sf::IntRect(0, 24, 28, 24));
                }
                else
                {
                    drzwi[i].setTextureRect(sf::IntRect(0, 0, 28, 24));
                }
            }
            else
            {
                drzwi[i].setTextureRect(sf::IntRect(28, 0, 28, 24));
            }
        }
        else if(i==1)
        {
...
void next_floor(hero *Hero, bool &boss_dead, int &actual_room_x, int &actual_room_y, bool &restart)
{
    boss_dead = 0;
    ...
    Zombie::base_attack += 10;
    Zombie::base_health += 50;

    Shooter::base_attack += 10;
    Shooter::base_health += 50;
    Shooter::base_spell_attack += 10;

    Boomer::base_attack += 10;
    Boomer::base_health += 50;

    grimm::base_attack += 20;
    grimm::base_spell_attack +=20;
    grimm::base_health += 100;

    Grimmchild::base_attack += 20;
    Grimmchild::base_spell_attack += 20;
    Grimmchild::base_health += 100;

    hollow_knight::base_attack += 20;
    hollow_knight::base_spell_attack += 20;
    hollow_knight::base_health += 100;
...
void boss_generator(std::vector<boss*> &Boss)
{
    switch(rand() % 2 + 1)  // uwazaj na rand()
        {
            case 1:
                Boss.push_back(new hollow_knight);
                break;
            case 2:
                Boss.push_back(new grimm);
                break;
        }
}
