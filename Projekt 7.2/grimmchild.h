...
public:
    ...
    void move_entity(float player_position_x, float player_position_y)
    {
        if (sqrt(pow(player_position_x - hitbox_pos_x, 2) + pow(player_position_y - hitbox_pos_y, 2) > pow(zasieg, 2)) ||(protection && speed_x <= 0))
        {
            ...
            // Lewo
            if (player_position_x + 43 / 2 < hitbox_pos_x && (get_pos_x() - speed_x > player_position_x) && pos_x - speed_x > 0 && pos_x - speed_x <= 215)
            {
                position(get_pos_x() - speed_x, get_pos_y());
                if (direction != 0 && !protection)  // Jezeli nie skrecil w lewo
                {
                    direction = 0; // Skrec w lewo
                    texture(grimmchild_dir, 0, 57, sizeX, sizeY);
                    fly_timer.restart();
                }
                if (frame && fly_timer.getElapsedTime().asMilliseconds() > fly_animation_time)
                {
                    texture(grimmchild_dir, 0, 57, sizeX, sizeY);  // Podnoszenie skrzydel(Lewo)
                    frame = 0;
                    fly_timer.restart();
                }
                else if (!frame && fly_timer.getElapsedTime().asMilliseconds() > fly_animation_time)
                {
                    texture(grimmchild_dir, 55, 57, sizeX, sizeY);  // Opuszczanie skrzydel(Lewo)
                    frame = 1;
                    fly_timer.restart();
                }
            }
            // Prawo
            else if (player_position_x + 43 / 2 > hitbox_pos_x && (get_pos_x() + speed_x < player_position_x) && pos_x + speed_x > 0 && pos_x + speed_x <= 215)
            {
                position(get_pos_x() + speed_x, get_pos_y());
                if (direction != 1 && !protection)  // Jezeli nie skrecil w prawo
                {
                    direction = 1; // Skrec w prawo
                    texture(grimmchild_dir, 0, 57, sizeX, sizeY);
                    fly_timer.restart();
                }
                if (frame && fly_timer.getElapsedTime().asMilliseconds() > fly_animation_time)
                {
                    texture(grimmchild_dir, 0, 0, sizeX, sizeY);  // Podnoszenie skrzydel(Prawo)
                    frame = 0;
                    fly_timer.restart();
                }
                else if (!frame && fly_timer.getElapsedTime().asMilliseconds() > fly_animation_time)
                {
                    texture(grimmchild_dir, 55, 0, sizeX, sizeY);  // Opuszczanie skrzydel(Prawo)
                    frame = 1;
                    fly_timer.restart();
                }
            }
        ...
        {
            if(!protection) // Potwor nie stal w miejscu
            {
...
{
    texture(grimmchild_dir, 0, 0, sizeX, sizeY);
    wartosc = 1;
    zasieg = 100;
    size_x = 55;
    size_y = 57;
...
void Grimmchild::damaged()
{
    ...
    if (direction == 0) // Skrecil w lewo
    {
        texture(grimmchild_dir, 57, 114, sizeX, sizeY);
        fly_timer.restart();
    }
    else  // Skrecil w prawo
    {
        texture(grimmchild_dir, 0, 114, sizeX, sizeY);
        fly_timer.restart();
    }
}
// Potwor dropie monety
void Grimmchild::death(std::vector<pickups *> &pickups, int coin_chance, int key_chance)
{
    srand(time(NULL));
