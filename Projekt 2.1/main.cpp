...
int main()
{
    int Windowposx = 1920; //szerokosc okna
    int Windowposy = 1080; //wysokosc okna
    ...
    //tlo
    sf::RectangleShape podloga(sf::Vector2f(Windowposx,Windowposy));
    ...
    for(int i=0;i<4;i++) //skalowanie drzwi
    {
        drzwi[i].setScale(0.2,0.2);
    }
    drzwi[0].setPosition((Windowposx)/2,0);
    drzwi[1].setPosition(0,(Windowposy-50)/2);
    drzwi[2].setPosition(Windowposx-80,(Windowposy-50)/2);
    drzwi[3].setPosition((Windowposx)/2,Windowposy-110);
    ...
    RysujDrzwi(czyPokoj, czyDrzwiRysuj, aktualnyPokojx,aktualnyPokojy);
    for(int i=0;i<5;i++)
    {
        for(int j=0;j<5;j++)
        {
    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            if (event.type == sf::Event::Closed)
                window.close();
            if  (event.type == sf::Event::KeyPressed) //przycisk pauzy esc
            {
                if(event.key.code == sf::Keyboard::Escape)
                {
                    time=!time; //czas w grze stop
                }
            }
            for(int i=0;i<drzwi.size();i++)
            {
                if(bohater.getGlobalBounds().intersects(drzwi[i].getGlobalBounds())) //kolizja bohatera z drzwiami
                {
                    switch(i)
                    {
                        case 0:
                    ...
                    bohater.setTextureRect(sf::IntRect(14,6,14,27)); //ustawianie idle textury
                    RysujDrzwi(czyPokoj, czyDrzwiRysuj, aktualnyPokojx,aktualnyPokojy);
                }
            }
        }
        ...
        if(time)
        {
            if(sf::Keyboard::isKeyPressed(sf::Keyboard::Left) && bohaterPosx-0.5>0) //ruch w lewo
            {
                bohaterPosx=bohaterPosx-0.5;
                bohater.setTextureRect(sf::IntRect(14,93,15,27));
            }
            if(sf::Keyboard::isKeyPressed(sf::Keyboard::Up) && bohaterPosy-0.5>0) //ruch w gore
            {
                bohaterPosy=bohaterPosy-0.5;
            }
            if(sf::Keyboard::isKeyPressed(sf::Keyboard::Down) && bohaterPosy+0.5<Windowposy-100) //ruch w dol
            {
                bohaterPosy=bohaterPosy+0.5;
            }
            if(sf::Keyboard::isKeyPressed(sf::Keyboard::Right) && bohaterPosx+0.5<Windowposx-75) //ruch w prawo
            {
                bohaterPosx=bohaterPosx+0.5;
