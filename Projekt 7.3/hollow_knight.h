...
public:
    ...
    void boss_action(hero *Hero,std::vector<Bullet*> *bullets,int wall_size_x,int wall_size_y,std::vector<boss*> *Boss,std::vector<pickups *> &pickups,bool &boss_dead, std::vector<Entity*> &monsters);
};
...
hollow_knight::hollow_knight()
{
    ...
    boss_dir = "Textures/Hollow_knight.png"; //"/Users/mcwilan/Library/Mobile Documents/com~apple~CloudDocs/pro3/Projekt cpp/Textures/Hollow_knight.png"
...
void hollow_knight::boss_action(hero *Hero,std::vector<Bullet*> *bullets,int wall_size_x,int wall_size_y,std::vector<boss*> *Boss,std::vector<pickups*> &pickups,bool &boss_dead, std::vector<Entity*> &monsters)
{
    ...
    if(cast==1 && spell_time.getElapsedTime().asSeconds()<spell_time1)
    {
        ...
        {
            bullets->push_back(new Bullet(hitbox_pos_x + 5, hitbox_pos_y + 15, 5, 5, spell_damage,"Textures/Hollow_bullet.png",Hero->get_pos_x()+Hero->get_sizex()/2,Hero->get_pos_y()+Hero->get_sizey()/2));
            shoot_timer.restart();
        }
    }
    else if(cast==1)
