...
void grimm::boss_action(hero *Hero,std::vector<Bullet*> *bullets,int wall_size_x,int wall_size_y,std::vector<boss*> *Boss,std::vector<pickups*> &pickups,bool &boss_dead, std::vector<Entity*> &monsters)
{
    ...
    if(cast)
    {
        if(shoot_timer.getElapsedTime().asSeconds()>=shoot_time)
        {
            if(direction==1)
            {
                ...
                direction = 0;
                bullets->push_back(new Bullet(hitbox_pos_x - size_x/2, hitbox_pos_y, 22, 33, spell_damage,"Textures/Grimm_bullet.png",Hero->get_pos_x()+Hero->get_sizex()/2,Hero->get_pos_y()+Hero->get_sizey()/2));
            }
            else
            {
