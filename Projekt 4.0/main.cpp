    ...
    while (window.isOpen())
    {
        ...
        if(time)
        {
            ...
            for(size_t i=0;i<monsters.size();i++)
            {
                ...
                if(monsters[i]->damage && monsters[i]->attacked.getElapsedTime().asMilliseconds()%100 > 20)
                {
                    monsters[i]->set_speed(monsters[i]->get_speed()+0.04f/100);
                    if(monsters[i]->damage_ticks < 50)
                    {
                        monsters[i]->move_entity(Hero.get_pos_x(),Hero.get_pos_y());
                    }
                    if(monsters[i]->damage_ticks == 100)
                    {
                        monsters[i]->damage = false;
                    }
                    monsters[i]->damage_ticks++;
                    monsters[i]->attacked.restart();
                }
            }
            //  ///////////////////// LEWO(left 0, top 86) ////////////////////////////////////
...
/////////////////////////////////////// PRZEDMIOTY ////////////////////////////////////////
            ...
            {
                if(Hero.hitbox.getGlobalBounds().intersects(pickups[i]->pickup.getGlobalBounds()))
                {
                    if(pickups[i]->get_name() == "coin")
                    {
                        Hero.set_coinAmount(1);
                    }
                    else if(pickups[i]->get_name() == "key")
                    {
                        Hero.set_keyAmount(1);
                        cout<<Hero.get_keyAmount();
                    }
...
/////////////////////////////////////// ATAKI ///////////////////////////////////////////
            ...
            for(size_t i = 0; i < monsters.size(); i++)
            {
                if(Hero.attack_hitbox.getGlobalBounds().intersects(monsters[i]->hitbox.getGlobalBounds()) && Hero.get_attack() && !monsters[i]->get_protection())
                {
                    ...
                    monsters[i]->damaged();
                    monsters[i]->set_protection(true);
                    if(monsters[i]->get_health() == 0)
                    {
                        monsters[i]->death(pickups,Hero.get_coinChance(),Hero.get_keyChance());
...
//                    if(Zombie.direction != 0)  // Lewo
//                    {
//                        Zombie.direction = 0;
//                        Zombie.texture(zombie_dir, 86, 86, 43, 43);  // sprite akatujacy lewy
//                    }
//                    else  // Prawo
//                    {
//                        Zombie.direction = 0;
//                        Zombie.texture(zombie_dir, 43, 86, 43, 43);  // sprite akatujacy prawy
//                    }
                }
            }
            // /////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        if(!pickups.empty())
        {
            for(size_t i = 0; i < pickups.size(); i++)
            {
                window.draw(pickups[i]->pickup);
            }
        }
        for(size_t i = 0; i < monsters.size(); i++) //rysowanie potwor�w
        {
