...
class Entity{
private:
    std::string zombie_dir = "Textures/Zombie.png";
...
void Zombie::death(std::vector<pickups*> &pickups, int coin_chance, int key_chance)
{
    ...
    {
        pickups.push_back(new coin(1, hitbox_pos_x,hitbox_pos_y+size_y/1.5 - 10));
    }
    else if(chance > 50 && chance < 50+key_chance)
    {
        pickups.push_back(new key(1, hitbox_pos_x,pos_y+size_y/1.5 - 6));
    }
}
