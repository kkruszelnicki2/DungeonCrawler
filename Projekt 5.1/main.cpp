...
int main()
{
    ...
    const float wall_size_x = window_width/24;
    const float wall_size_y = window_height/13.5;
...
/////////////////////////////////////// PICKUPY ////////////////////////////////////////
            for(size_t i = 0;i < is_room[actual_room_x][actual_room_y].pickups.size(); i++)
            {
                ...
                {
                    if(is_room[actual_room_x][actual_room_y].pickups[i]->get_name() == "coin") //moneta
                    {
                        Hero.set_coinAmount(1);
                    }
                    else if(is_room[actual_room_x][actual_room_y].pickups[i]->get_name() == "key") //klucz
                    {
                        Hero.set_keyAmount(1);
                    }
                    is_room[actual_room_x][actual_room_y].pickups.erase(is_room[actual_room_x][actual_room_y].pickups.begin()+i); //cztszczenie z mapy podniesionego pickupa
                }
            }
/////////////////////////////////////// ATAKI ///////////////////////////////////////////
            ...
            for(size_t i = 0; i < monsters.size(); i++)
            {
                ...
                {
                    bullets.push_back(new Bullet(monsters[i]->hitbox_pos_x,monsters[i]->hitbox_pos_y,monsters[i]->get_spell_damage()));
                    bullets[bullets.size()-1]->set_trajectory(Hero.get_pos_x()+Hero.get_sizex()/2,Hero.get_pos_y()+Hero.get_sizey()/2);
                    monsters[i]->detonate = false;
                }
            }

            if(monsters.empty()) // dzialania po smierci ostatniego potwora
            {
                fight_mode = 0;
                for(int i = 0; i < 4; ++i)
                {
